...
(defun format-log (&rest args)
  "Send a message to syslog. If the first argument of ARGS is a
keyword it is assumed to be a non-default log level (:debug),
otherwise it is a control string followed by optional args (as in
FORMAT)."
  (destructuring-bind (level format-control &rest format-args)
...
(define-compiler-macro format-log (&whole whole &rest args)
  (when (keywordp (first args))
    ;; Sanity check that it's a valid log level at compile
    ;; time.
    (cl-syslog:get-priority (first args)))
  whole)
