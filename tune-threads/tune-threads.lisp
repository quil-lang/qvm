...
(defun time-hadamard (&key (num-trials 1) (num-threads 4) (num-qubits 23))
  "Time the Hadamard program and return the number of second per thread."
  ...
  (let* ((q (prepare-hadamard-test num-qubits))
         (run-time (simple-time num-trials (run-and-reset-qvm q))))
    (/ run-time (* num-threads num-trials))))

(defun scan-num-threads (&key (num-trials 1) (num-qubits 23)
                              (max-num-threads (qvm:count-logical-cores)) (min-num-threads 1))
  (loop :for i :from min-num-threads :to max-num-threads
