...
(deftest test-match-measure-at-any ()
  (let* ((program "DECLARE R0 BIT; X 0; CNOT 0 1; MEASURE 0 R0")
    ...
    (is (not (funcall (qvm::match-measure-at-any 0 1) x)))
...
(deftest test-match-any-gates ()
  (let* ((program "DECLARE R0 BIT; X 0; CNOT 0 1; MEASURE 0 R0")
    ...
    (is (not (funcall (qvm::match-any-gates "X") measure)))
...
(deftest test-match-strict-qubits ()
  (let* ((program "DECLARE R0 BIT; X 0; CNOT 0 1; MEASURE 0 R0")
    ...
    (is (not (funcall (qvm::match-strict-qubits 0) measure)))
...
(deftest test-match-any-n-qubits ()
  (let* ((program "DECLARE R0 BIT; X 0; CNOT 0 1; MEASURE 0 R0")
    ...
    (is (not (funcall (qvm::match-any-n-qubits 1 '(0 1)) measure)))
...
(deftest test-make-noise-predicate ()
  (let* ((pred (qvm::match-strict-qubits 0))
         ...
         (noise-pred (qvm::make-noise-predicate pred priority noise-position)))
    (is (qvm::priority noise-pred) 1)
    (is (qvm::noise-position noise-pred) :after)))


(deftest test-predicate-and ()
  (let* ((p1 (qvm::make-noise-predicate (qvm::match-strict-qubits 0) 1 :after))
         (p2 (qvm::make-noise-predicate (qvm::match-strict-gate "X") 2 :before))
    ...
    (is (funcall (qvm::predicate-function and-p) test-gate-x0))
    (is (not (funcall (qvm::predicate-function and-p) test-gate-x1)))
    (is (string= (qvm::name and-p) "(NOISE-NAME & NOISE-NAME)"))))
...
(deftest test-predicate-or ()
  (let* ((p1 (qvm::make-noise-predicate (qvm::match-strict-qubits 0) 1 ':after))
    ...
    (is (funcall (qvm::predicate-function or-p) test-gate-1))
    (is (funcall (qvm::predicate-function or-p) test-gate-2))
    (is (not (funcall (qvm::predicate-function or-p) test-gate-3)))
    (is (string= (qvm::name or-p) "(NOISE-NAME | NOISE-NAME)"))))


(deftest test-predicate-not ()
  (let* ((p (qvm::make-noise-predicate (qvm::match-strict-qubits 0) 1 ':after))
    ...
    (is (funcall (qvm::predicate-function not-p) test-gate-1))
    (is (not (funcall (qvm::predicate-function not-p) test-gate-2)))
    (is (string= (qvm::name not-p) "!NOISE-NAME"))))


(deftest test-make-noise-rule ()
  (let* ((np (qvm::make-noise-predicate (qvm::match-strict-qubits 0) 1 ':after))
...
(defun pred-1 ()
  (qvm::make-noise-predicate (qvm::match-strict-qubits 0) 1 ':after))

(defun pred-3 ()
  (qvm::make-noise-predicate (qvm::match-all-nq-gates 1) 5 ':after))

(defun pred-2 ()
  (qvm::make-noise-predicate (qvm::match-strict-gate "X") 2 ':after))
...
(deftest test-multiply-noise-models-simple ()
  ...
  (let* ((priority 20)
         (p1 (qvm::make-noise-predicate (qvm::match-strict-qubits 0) priority ':after))
         (p2 (qvm::make-noise-predicate (qvm::match-strict-qubits 1) priority ':after))
    ...
    ;; and p1's elems).
    (let ((first-rule (first (qvm::noise-rules mult-nm)))
          (second-rule (second (qvm::noise-rules mult-nm)))
          (third-rule (third (qvm::noise-rules mult-nm))))
